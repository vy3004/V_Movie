      ...
      >
        {hasCamera ? (
          ...
          <ParticipantStatusAvatar
            ...
            fullName={participantIdentity.fullName}
          />
        )}
      </SpeakingEffect>
...
const ParticipantStatusAvatar = React.memo(function ParticipantStatusAvatar({
  userId,
  avatarUrl,
  fullName,
}: {
  ...
  const presence = useWatchPartyStore(
    useCallback((state) => state.presenceData[userId] ?? null, [userId]),
  );

  const status = !presence ? "offline" : presence.status === "away" ? "away" : "online";

  return (
    <UserAvatar avatar_url={avatarUrl} user_name={fullName} size={40} status={status} />
  );
});
