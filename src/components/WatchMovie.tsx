...
import { Movie, HistoryItem, EpisodeProgress, Episode, ServerData } from "@/lib/types";
import { getLocalHistory, saveLocalHistory } from "@/lib/utils";
...
export default function WatchMovie({ movie, history, user }: Props) {
  ...
  const [sessionProgress, setSessionProgress] = useState<Record<string, EpisodeProgress>>({});
  const [isInitializing, setIsInitializing] = useState(true);
  const playbackRef = useRef({ currentTime: 0, duration: 0 });

  const allEpisodes = useMemo(() => movie.episodes.flatMap((ep: Episode) => ep.server_data), [movie.episodes]);
  const lastEpisodeSlug = useMemo(() => allEpisodes[allEpisodes.length - 1]?.slug, [allEpisodes]);

  // Sử dụng hook đã tối ưu
  const { handleTimeUpdate, syncToSupabase } = useWatchHistory({ user, movie, episodeSlug: tap || "" });

  const { nextEpSlug, prevEpSlug } = useMemo(() => {
    const idx = allEpisodes.findIndex((s) => String(s.slug) === String(tap));
    return {
      nextEpSlug: idx !== -1 && idx < allEpisodes.length - 1 ? allEpisodes[idx + 1].slug : null,
  ...
  useEffect(() => {
    const loadInitialHistory = () => {
      ...
      if (!user && !history) {
        const localData = getLocalHistory().find((h) => h.movie_slug === movie.slug);
        if (localData?.episodes_progress) initialProgress = localData.episodes_progress;
      }
      setSessionProgress(initialProgress);
      setIsInitializing(false);

      if (!tap && allEpisodes.length > 0) {
        const localHist = getLocalHistory().find((h) => h.movie_slug === movie.slug);
        const targetEp = user ? history?.last_episode_slug : localHist?.last_episode_slug;
  ...
  const resumeTime = useMemo(() => {
    ...
    if (!epData) return 0;
    if (epData.ep_duration > 0 && epData.ep_last_time / epData.ep_duration > 0.98) return 0;
  ...
  const handleSelectEpisode = useCallback((sv: ServerData) => {
    syncToSupabase(); 
  ...
  if (isInitializing || !tap) return <div className="h-[60vh] flex items-center justify-center text-zinc-500">Đang tải...</div>;
      ...
      <div id="video" className="scroll-mt-24">
        {activeEpisode && (
          <VideoPlayer
            ...
            onProgress={handleTimeUpdate} // Gắn trực tiếp vào handleTimeUpdate của hook
            onAutoNext={handleAutoNext}
          />
        )}
      </div>
      <div className="bg-background p-6 rounded-xl border border-zinc-800">
        <h3 className="text-lg font-bold text-white mb-4 uppercase tracking-wider">Danh sách tập</h3>
        <EpisodeSelector
