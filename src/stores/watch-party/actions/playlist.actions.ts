 ...
 */
const pendingPlaylistAdds = new Set<string>();
...
export async function addMovieToPlaylist(
  ...
  const pendingKey = `${roomId}:${selectedSlug}:${firstEpisodeSlug}`;

  if (pendingPlaylistAdds.has(pendingKey)) return;
  pendingPlaylistAdds.add(pendingKey);
  ...
  try {
    ...
    if (res.ok) {
      ...
      toast.error(await getResponseError(res, "Không thể thêm phim vào danh sách"));
    }
  } catch {
    toast.error("Lỗi kết nối, vui lòng thử lại");
  } finally {
    pendingPlaylistAdds.delete(pendingKey);
  }
}
...
export async function playPlaylistItemNow(
  ...
  try {
    ...
    // Player state reset handled by WatchPartyRealtime on episode change
    ...
    if (!deleteRes.ok) {
      toast.error(
        await getResponseError(
          deleteRes,
          "Đã chuyển phim nhưng chưa xóa được khỏi danh sách chờ",
        ),
      );
    }

    // Broadcast episode change through existing data channel
    await store.dataChannel?.send({
      type: "broadcast",
    ...
    fetch("/api/watch-party/sync", {
      ...
      if (!res.ok) {
        console.error(
          "[Playlist] Failed to sync playlist playback:",
          await getResponseError(res, "Không thể đồng bộ trạng thái phát"),
        );
      }
    }).catch((error) => {
      console.error("[Playlist] Failed to sync playlist playback:", error);
    });
